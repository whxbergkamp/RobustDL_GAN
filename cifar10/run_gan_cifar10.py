...
from adversarial_networks.train_gan_cifar10 import train
...
parser.add_argument('--g-architecture', default='resnet18_2', type=str, help='Architecture for generator.')
...
parser.add_argument('--log-dir', default='./logs_cifar10', type=str, help='Where to store log and checkpoint files.')
parser.add_argument('--model-fileD', default='./modelD_cifar10.ckpt', type=str, help='Model file for D.')
parser.add_argument('--model-fileG', default='./modelG_cifar10.ckpt', type=str, help='Model file for G.')
...
def main():
    ...
    filename_queue = get_filename_queue(
        split_file=os.path.join(args.data_dir, 'splits', args.dataset, 'train.lst'),
    ...
    generator = models.get_generator(args.g_architecture,
        output_size=output_size, c_dim=args.c_dim, f_dim=args.gf_dim)
