...
def train(discriminator, data, test_data, config):
    ...
    optimizer2 = tf.train.MomentumOptimizer(learning_rate=learning_rate/10, momentum=0.9)

    d_grads = tf.gradients(d_loss_adv + d_decay, d_vars)
    ...
    with sv.managed_session() as sess:
        ...
        for batch_idx in range(config['nsteps']):
            ...
            if batch_idx<100000:
                d_loss_out, d_reg_out, _ = sess.run([d_loss_adv, d_reg, train_op])
            else: 
                d_loss_out, d_reg_out, _ = sess.run([d_loss_adv, d_reg, train_op2])
            ...
            if batch_idx % num_steps_per_epoch==0:
                ...
                % (batch_idx, d_loss_out, test_acc, test_acc_fgs, test_acc_pgd, d_reg_out), flush=True)
