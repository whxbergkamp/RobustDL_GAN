...
def train(discriminator, config, train_image, train_label, test_image, test_label):
    ...
    d_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        logits=d_out_real, labels=tf.one_hot(label,10)))
    ...
    acc_fgs, acc_update_fgs, acc_init_fgs = build_test_fgs(discriminator, test_image_placeholder, test_label_placeholder, config)
    acc_pgd, acc_update_pgd, acc_init_pgd = build_test_pgd(discriminator, test_image_placeholder, test_label_placeholder, config)
    ...
    with sv.managed_session() as sess:
        ...
        for batch_idx in range(config['nsteps']):
            ...
            if batch_idx<100000:
                d_loss_out, d_reg_out, _ = sess.run([d_loss, d_reg, train_op], feed_dict={image_placeholder: image_minibatch, label_placeholder: label_minibatch})
            elif batch_idx<150000:
                d_loss_out, d_reg_out, _ = sess.run([d_loss, d_reg, train_op2], feed_dict={image_placeholder: image_minibatch, label_placeholder: label_minibatch})
            else:
                d_loss_out, d_reg_out, _ = sess.run([d_loss, d_reg, train_op3], feed_dict={image_placeholder: image_minibatch, label_placeholder: label_minibatch})


            if batch_idx % 100==0:
                test_acc = run_test(acc, acc_update, acc_init, sess, config, test_image, test_label, test_image_placeholder, test_label_placeholder)
                test_acc_fgs = run_test(acc_fgs, acc_update_fgs, acc_init_fgs, sess, config, test_image, test_label, test_image_placeholder, test_label_placeholder)
                test_acc_pgd = run_test(acc_pgd, acc_update_pgd, acc_init_pgd, sess, config, test_image, test_label, test_image_placeholder, test_label_placeholder)
