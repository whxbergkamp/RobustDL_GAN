...
def train(generator, discriminator, data, test_data, config):
    ...
    d_loss_noise = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        logits=d_out_noise, labels=tf.one_hot(label,10)))
    ...
    with sv.managed_session() as sess:
        ...
        for batch_idx in range(config['nsteps']):
            ...
            # run more G steps (5 for every D step)
            if batch_idx<100000:
                if batch_idx % 5 == 0: 
                    d_loss_out, g_loss_out, g_loss3_out, g_loss5_out, d_reg_out, g_reg_out, _, _ = sess.run([d_loss, g_loss, g_loss3, g_loss5, d_reg, g_reg, g_train_op5, d_train_and_restore_op])
                else:
                    d_loss_out, g_loss_out, g_loss3_out, g_loss5_out, d_reg_out, g_reg_out, _ = sess.run([d_loss, g_loss, g_loss3, g_loss5, d_reg, g_reg, g_train_op5])
            else:
                if batch_idx % 5 == 0:
                    d_loss_out, g_loss_out, g_loss3_out, g_loss5_out, d_reg_out, g_reg_out, _, _ = sess.run([d_loss, g_loss, g_loss3, g_loss5, d_reg, g_reg, g_train_op5, d_train_and_restore_op2])
                else:
                    d_loss_out, g_loss_out, g_loss3_out, g_loss5_out, d_reg_out, g_reg_out, _ = sess.run([d_loss, g_loss, g_loss3, g_loss5, d_reg, g_reg, g_train_op5])
