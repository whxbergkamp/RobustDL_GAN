# experiment on standard undefended model
import numpy as np
import argparse

from adversarial_networks.train_standard_svhn import train
import pprint
...
parser = argparse.ArgumentParser(description='Train standard undefended model.')
# Architecture
...
parser.add_argument('--log-dir', default='./logs_std_svhn', type=str, help='Where to store log and checkpoint files.')
parser.add_argument('--model-file', default='./model_std_svhn.ckpt', type=str, help='checkpoint file for fgs model.')
...
def main():
    ...
    train(discriminator, config, image_combined, label_combined, test_image_npy, test_label_npy)
